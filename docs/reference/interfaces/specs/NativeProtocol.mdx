...
| QUERY_PLAN_SERIALIZATION        | 54477   | ServerHello, QueryPlan packet | ServerHello appends `VarUInt query_plan_serialization_version` after server settings. Also introduces `ClientPacket::QueryPlan` (code `13`) for inter-server delivery of pre-built query plans — external clients never send. Query-plan serialization version `11` prefixes the serialized plan with `VarUInt max_threads` and `Bool concurrency_control`; when either is non-default, initiators use SQL shipping for peers below version `11` so the peer builds its plan from the transmitted query settings. Version `18` appends `VarUInt min_frame_rows_for_aggregate_tree` (the `min_window_frame_rows_for_aggregate_tree` setting of the initiator) to the serialized `Window` step; below `18` the field is absent on the wire, and a reader at `18` or above treats such a step as having the aggregate tree disabled. |
